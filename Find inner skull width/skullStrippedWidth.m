...
for frame = 1:size(Img, 3)
    ...
    centerX = 75; % Center x-coordinate of the circle, near the origin
    centerY = 128; % Center y-coordinate of the circle, near the origin
    radius = 15; % Radius of the circle

    % Initialize the level set function (LSF)
    initialLSF = c0 * ones(size(Img_frame)); % Initializes outside the circle

    % Set values inside the circle to -c0
    initialLSF((x - centerX).^2 + (y - centerY).^2 <= radius^2) = -c0;
    ...
    for n=1:iter_outer
        ...
        phi = strippedInnerSkull(phi, Img_smooth, lambda, mu, alfa, epsilon, timestep, iter_inner);
        % Plot the updated contour
        imagesc(Img_frame, [0, 1000]); axis off; axis equal; colormap(gray); hold on;
        hContour = contour(phi, [0, 0], 'r', 'LineWidth', 2);
        % Update title
