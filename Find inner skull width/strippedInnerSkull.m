function phi = strippedInnerSkull(phi_0, g, lambda,mu, alfa, epsilon, timestep, iter)
    ...
    for k=1:iter
        ...
        regularTerm=adaptedLevelSet(phi, vx, vy);  % use phi and img gradient
        % diracPhi=Dirac(phi,epsilon);
        edgeTerm = smoothTerm(phi, curvature);  % use phi and curvature (LSF gradient)
        stopOnEdge = edgeStopper(phi, vx, vy);
    
        delta_phi = 0.1*(-1+stopOnEdge);
        phi=phi + timestep*(delta_phi);
        % phi=phi + timestep*(edgeTerm + regularTerm);
    end

    function f = edgeStopper(phi, vx, vy) 
        % Initialize f with zeros, assuming phi is a matrix
        f = zeros(size(phi));
        % Create a mask for pixels near the level set
        nearLevelSet = abs(phi) < 1.5;

        magGrad = sqrt(vx.^2 + vy.^2);

        % Apply logistic sigmoid to phi values near the level set
        f(nearLevelSet) = logsig(magGrad(nearLevelSet));
    ...
    function f = smoothTerm(phi, curvature)
